...
有没有想过在手机或者平板的obsidian上直接运行你笔记中的代码？我常常想，obsidian写出来的代码块这么漂亮，不能直接跑就太可惜了。如果这些代码块能够随时实现输入与输出，这个笔记软件几乎就是一个完美的移动小终端了。我尝试了一些其他插件，发现它们很少同时兼顾移动端可用、输入功能与输出功能，于是我基于 Code Emitter 加入了自己想要的这些功能，并按自己的需求做了一些改动，最终做出了这个小插件。

你可以随时随地拿起任何一台设备，打开obsidian开始运行代码。点击代码块右下角左侧的小键盘按钮后，就可以进行输入了；点击右侧的运行按钮，就可以看到代码的运行结果了。这不是很有意思吗？

后续还将更新语言切换功能和自动输入识别功能，也许还会和其他插件组合使用，实现更多功能。

---

Have you ever thought about running the code in your notes directly in Obsidian on your phone or tablet? I often think it's such a pity that the code blocks made in Obsidian look so beautiful but can't be run directly. If these code blocks could handle input and output anytime, this note-taking app would be almost a perfect mobile terminal. I tried some other plugins and found that few of them balance mobile usability, input functionality, and output functionality all at once. So I took Code Emitter, added the features I wanted, made some adjustments to suit my needs, and finally built this little plugin.

You can pick up any device, open Obsidian, and start running code anytime, anywhere. Click the keyboard button on the left at the bottom of the code block to enter input; click the run button on the right to see the result. Isn't that interesting?
